...
if params["data"].get("sample_size"):
    df = df.sample(
        n=params["data"]["sample_size"],
        random_state=params["model"]["random_state"]
    )
df = df[[text_column, label_column]].dropna()

# changing label into binary feature
if params["data"]["label_type"] == "amazon":
    df['label'] = df[label_column].apply(lambda x: 1 if x >= 4 else 0)
elif params["data"]["label_type"] == "twitter":
    df['label'] = df[label_column].map({0: 0, 4: 1})
else:
...
model_path = f"models/{vec_type}_{params['model']['type']}.pkl"
